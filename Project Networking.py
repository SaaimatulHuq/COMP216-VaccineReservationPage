...
class Database:
    ...
    def Search(self, id):
        ...
        # searchResults = self.dbCursor.fetchone()
    ...
    def sortedDisplay(self,col,order):
        print(col)
        self.dbCursor.execute("SELECT * FROM patient_info ORDER BY {} {} ".format(col,order))
        records = self.dbCursor.fetchall()
        return records
...
class InsertWindow:
    def __init__(self):
        ...
        self.yearList = list(range(2021, 2025))
        ...
        tkinter.Label(self.window, text = "Date of Reservation",  width = 25).grid(pady = 5, column = 1, row = 4)
        tkinter.Label(self.window, text = "Month of Reservation",  width = 25).grid(pady = 5, column = 1, row = 5)
        tkinter.Label(self.window, text = "Year of Reservation",  width = 25).grid(pady = 5, column = 1, row = 6)
    ...
    def Insert(self):
        ...
        if (self.test == "SUCCESS"):
            # if self.database.Search(self.idEntry.get()) is None:
            ...
            # else:
            #     tkinter.messagebox.showinfo("Failed to insert data", "Patient Id already exists in the database")
        else:
...
class UpdateWindow:
    def __init__(self, id):
        ...
        self.dateList = list(range(1, 32))
        self.monthList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
        self.yearList = list(range(2021, 2027))
        self.bloodGroupList = ["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"]
...
class SortedDisplay:
    def __init__(self):
        self.window = tkinter.Tk()
        self.window.wm_title("Sorted Display")
        self.colList = ["id", "fName", "lName", "dob", "mob", "yob", "gender", "address", "phone", "email","bloodGroup", "history", "doctor"]
        self.orderList=["ASC","DESC"]
        # Label
        tkinter.Label(self.window, text="Column to sort ", width=50).grid(pady=5,column=1, row=1)
        tkinter.Label(self.window, text="Order", width=50).grid(pady=5, column=1, row=2)
        # Entry widgets
        self.column = tkinter.ttk.Combobox(self.window, values=self.colList, width=20)
        self.column.grid(pady=10, column=2, row=1)
        self.order = tkinter.ttk.Combobox(self.window, values=self.orderList, width=20)
        self.order.grid(pady=10, column=2, row=2)
        # Button widgets
        tkinter.Button(self.window, width=20, text="Display", command=self.display).grid(pady=10, column=2,row=3)
        self.window.mainloop()
    def display(self):
        self.database = Database()
        self.data = self.database.sortedDisplay(self.column.get(),self.order.get())
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView.heading("dob", text = "Date")
        self.databaseView.heading("mob", text = "Month")
        self.databaseView.heading("yob", text = "Year")
...
class SearchDeleteWindow:
    ...
    def Delete(self):
        ...
        tkinter.messagebox.showinfo("Deleted data", "Successfully deleted the above data in the database")


class HomePage:
    def __init__(self):
        self.homePageWindow = tkinter.Tk()
        self.homePageWindow.wm_title("Vaccine Reservation System")

        tkinter.Label(self.homePageWindow, text = "COVID-19 Vaccine Reservation",  width = 100).grid(pady = 20, column = 1, row = 1)

        tkinter.Button(self.homePageWindow, width = 20, text = "Insert Patient Information", command = self.Insert).grid(pady = 15, column = 1, row = 2)
        tkinter.Button(self.homePageWindow, width = 20, text = "Update Patient Information", command = self.Update).grid(pady = 15, column = 1, row = 3)
        
        tkinter.Button(self.homePageWindow, width = 20, text = "Delete Patient Information", command = self.Delete).grid(pady = 15, column = 1, row = 5)
        tkinter.Button(self.homePageWindow, width = 20, text = "Display Patient Information", command = self.Display).grid(pady = 15, column = 1, row = 6)

        tkinter.Button(self.homePageWindow, width = 20, text = "Search Patient Information", command = self.Search).grid(pady = 15, column = 1, row = 4)
        tkinter.Button(self.homePageWindow, width=20, text="Advanced Search", command=self.SortedDisplay).grid(pady=15, column=1,row=7)
        tkinter.Button(self.homePageWindow, width = 20, text = "Exit System", command = self.homePageWindow.destroy).grid(pady = 15, column = 1, row = 8)
    ...
    def SortedDisplay(self):
        self.sortedDisplay=SortedDisplay()
